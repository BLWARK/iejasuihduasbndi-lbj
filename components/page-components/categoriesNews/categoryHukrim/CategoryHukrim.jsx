...
// Ambil berita Hukum & Kriminal, urutkan berdasarkan tanggal terbaru
const hukrimNews = newsData
  .filter((news) => news.category.some((cat) => cat.toLowerCase() === "hukum & kriminal"))
  .sort((a, b) => new Date(b.date) - new Date(a.date));

// Ambil satu berita utama dan sisanya untuk list kecil
const mainNews = hukrimNews[0]; // Berita utama
const smallNews = hukrimNews.slice(1, 4); // Berita kecil

const CategoryHukrim = () => {
  return (
    <div className="flex flex-col">
      {/* Header Kategori */}
      <div className="flex flex-col items-start gap-3 mb-4">
        <h2 className="text-2xl font-bold text-red-600">Hukum & Kriminal</h2>
        <div className="w-[60px] h-[5px] bg-red-600"></div>
      </div>
