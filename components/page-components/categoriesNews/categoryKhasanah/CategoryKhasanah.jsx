...
// Ambil berita Khasanah, urutkan berdasarkan tanggal terbaru
const khasanahNews = newsData
  .filter((news) => news.category.some((cat) => cat.toLowerCase() === "khasanah"))
  .sort((a, b) => new Date(b.date) - new Date(a.date));

// Ambil satu berita utama dan sisanya untuk list kecil
const mainNews = khasanahNews[0]; // Berita utama
const smallNews = khasanahNews.slice(1, 4); // Berita kecil

const CategoryKhasanah = () => {
  return (
    <div className="flex flex-col">
      {/* Header Kategori */}
      <div className="flex flex-col items-start gap-3 mb-4">
        <h2 className="text-2xl font-bold text-green-600">Khasanah</h2>
        <div className="w-[60px] h-[5px] bg-green-600"></div>
      </div>
      ...
      <div className="flex flex-col gap-6">
        {smallNews.length > 0 ? (
          smallNews.map((news) => (
            <div key={news.id} className="flex gap-6 items-start">
...
export default CategoryKhasanah;
